...
fn extract_user_data() -> LlmFunction<UserData> {
    // TODO: For now, create LLM here. Will be refactored once we finalize skill design
...
impl SkillHandler for SummarizeResumeSkill {
    ...
    async fn on_request(
        ...
        runtime.memory_service().save("resumes", &user_data).await?;
    ...
    async fn on_input_received(
        ...
        let slot: ResumeInputSlot = task_context.get_input_slot()?.unwrap();

        match slot {
            ResumeInputSlot::CorrectedUsername => {
                let partial_user: UserData = task_context
                    .load_data("partial_user")?
                    .ok_or_else(|| anyhow!("No partial user data found"))?;
                ...
                if repos.is_empty() {
                    Ok(OnInputResult::Failed {
                        error: "Could not find repositories with the provided username."
                            .to_string(),
                    })
                } else {
                    let user = FinalUser {
