//! Anthropic Claude Simple Conversation Tests
//!
...
async fn test_anthropic_simple_conversation_comprehensive() {
    let Some(agent) = create_test_agent() else {
        println!("⚠️  Skipping test: ANTHROPIC_API_KEY not found");
        return;
    };

    println!("🧪 Testing Anthropic simple conversation with comprehensive validation...");
...
#[tokio::test]
#[ignore] // Only run with --ignored flag when API key is available
async fn test_anthropic_streaming_comprehensive() {
    let Some(agent) = create_test_agent() else {
        println!("⚠️  Skipping test: ANTHROPIC_API_KEY not found");
        return;
    };

    println!("🧪 Testing Anthropic streaming with comprehensive A2A event validation...");
...
async fn test_anthropic_multi_turn_conversation() {
    ...
    println!("🧪 Testing Anthropic multi-turn conversation with session continuity...");

    // ✅ First turn - create new session
    let message1 = create_user_message("Hello! My name is Alice. What's 5 + 3?");
    ...
    let message3 = Message {
        ...
        role: MessageRole::User,
        parts: vec![Part::Text {
            text: "Can you summarize our conversation so far?".to_string(),
            metadata: None,
        }],
    ...
    let _task3 = match result3.unwrap().result {
        SendMessageResult::Task(task) => task,
        _ => panic!("Expected Task result"),
    };
