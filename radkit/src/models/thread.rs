...
impl Thread {
    ...
    #[must_use] pub const fn new(events: Vec<Event>) -> Self {
        Self {
            events,
            system: None,
        }
    ...
    #[must_use] pub fn system(&self) -> Option<&str> {
    ...
    #[must_use] pub fn into_events(self) -> Vec<Event> {
        self.events
    }

    /// Consumes the thread and returns both system prompt and events.
    #[must_use] pub fn into_parts(self) -> (Option<String>, Vec<Event>) {
        (self.system, self.events)
    }

    /// Converts the thread into a prompt string.
    #[must_use] pub fn to_prompt(&self) -> String {
