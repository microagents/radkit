...
mod tests {
    ...
    #[test]
    fn extract_structured_output_returns_value_and_remaining_content() {
        let structured_call =
            ToolCall::new("call-1", STRUCTURED_OUTPUT_TOOL_NAME, json!({ "value": 7 }));

        let content = Content::from_parts(vec![
            ContentPart::Text("Computation completed.".to_string()),
            ContentPart::ToolCall(structured_call),
        ]);

        let (value, assistant_content) =
            extract_structured_output::<Sample>(content).expect("structured output");

        assert_eq!(value, Sample { value: 7 });

        let assistant = assistant_content.expect("assistant message preserved");
        assert_eq!(
            assistant.joined_texts().as_deref(),
            Some("Computation completed.")
        );
    }

    #[test]
    fn extract_structured_output_without_tool_call_fails() {
        let content = Content::from_text("No structured data");
        let error = extract_structured_output::<Sample>(content).expect_err("should fail");
    ...
    async fn build_structured_output_toolset_exposes_single_tool() {
        let toolset = build_structured_output_toolset::<Sample>()
            .expect("toolset should be created successfully");

        let tools = toolset.get_tools().await;
        assert_eq!(tools.len(), 1);

        let declaration = tools[0].declaration();
        assert_eq!(declaration.name(), STRUCTURED_OUTPUT_TOOL_NAME);
